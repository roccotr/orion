...
void vatest (string sTablename, ...) {
	va_list cVlist;
	va_start (cVlist, sTablename);

	printf ("ok inizio a leggere i parametri\n");
	while (true) {
		int iVcoltype = va_arg (cVlist, int);
		printf ("letto %i\n", iVcoltype);
		if (iVcoltype < 0) break;
		string sVthiscolumn = va_arg (cVlist, char*);
		printf ("valore %i %s\n", iVcoltype, sVthiscolumn.c_str ());
...
#define iCbenchmarkcreatetableL2
#ifdef iCbenchmarkcreatetableL2
	{
		...
		struct StorageOptions storageOptions (L2);
		db->CreateTableConfig (storageOptions, "TabellaPeppeMas", 
			STRINGTYPE, "colonna1", 
			STRINGTYPE, "colonna2", 
			INTEGRTYPE, "colonnaPeppeMas",
			DB::VaEOF);
		db->Create (storageOptions);		
		struct OsqlOptions osqlOptions;
		s = db->Osql (osqlOptions, "INSERT INTO TABELLAPEPPEMAS (COLONNA1, COLONNA2, COLONNAPEPPEMAS) VALUES ('TEST', 'TEST2', 35)");
		s = db->Osql (osqlOptions, "INSERT INTO TABELLAPEPPEMAS (COLONNA1, COLONNA2, COLONNAPEPPEMAS) VALUES ('TEST2', 'PROVA', 35)");
		if (s.ok ()) printf ("insert riuscita\n");

		s = db->Osql (osqlOptions, "SELECT FROM TABELLAPEPPEMAS");
		
		for (int iV = 0; iV < s.snapshot_L2.cvkeyslice_size (); iV++) {
			printf ("chiave (osql): %s  - %s\n", 
				s.snapshot_L2.mutable_cvkeyslice (iV)->mutable_cvkey ()->svmain ().c_str (),
				s.snapshot_L2.mutable_cvkeyslice (iV)->mutable_cvcolumns (0)->mutable_cvvalue ()->svvalue ().c_str());
		}
